...
class IndustryreportsController extends HomeController
{
	...
	{	
		...
		return view('knowledgebank.pharmareport.index',compact('industryreports','banners','data_count'));	
	}

	public function show($url)
	{	
		$industryreports = IndustryReport::where('url',$url)->where('active_flag',1)->orderBy('id','desc')->select('title1','title2','full_report','id','short_description','created_at','updated_at')->first();
         if(empty($industryreports)){
            return view('errors.404');
        }
		$banners = $this->banners;
		$this->seoToolsView($industryreports->id); 
		return view('knowledgebank.pharmareport.show',compact('industryreports','banners'));	
	}
	...
	public function more($offset){
		...
		return response(view('knowledgebank.pharmareport.loadmore', compact('data'))->render());
