...
class WhitepapersController extends HomeController
{
  ...
  { 
    ...
    SEOMeta::setTitle($whitepaper->whf_meta_title);
    	SEOMeta::setDescription($whitepaper->whf_meta_description);
    	SEOMeta::addMeta('Project:published_time', $whitepaper->created_at->toW3CString(), 'property');
    	SEOMeta::addKeyword($whitepaper->whf_meta_keywords);
    	OpenGraph::setDescription($whitepaper->whf_og_description);
    	OpenGraph::setTitle($whitepaper->whf_og_title);
    	OpenGraph::setUrl(url()->current());
    	SEOMeta::setCanonical(url()->current());
    	OpenGraph::addProperty('keyword', $whitepaper->whf_og_keywords);
    	OpenGraph::addProperty('type', 'Project');
    	OpenGraph::addProperty('locale', 'en_GB');
            //OpenGraph::addProperty('locale:alternate', ['pt-pt', 'en-us']);
    	OpenGraph::addImage([$whitepaper->whf_og_image, 'size' => 300]);
