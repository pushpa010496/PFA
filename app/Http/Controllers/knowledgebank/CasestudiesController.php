...
class CasestudiesController extends HomeController
{
  ...
  { 
    ...
    $casestudy = CaseStudy::where('url', $url)->where('active_flag', 1)->orderBy('id', 'desc')->first();
         SEOMeta::setTitle($casestudy->csf_meta_title);
    	SEOMeta::setDescription($casestudy->csf_meta_description);
    	SEOMeta::addMeta('Project:published_time', $casestudy->created_at->toW3CString(), 'property');
    	SEOMeta::addKeyword($casestudy->csf_meta_keywords);
    	OpenGraph::setDescription($casestudy->csf_og_description);
    	OpenGraph::setTitle($casestudy->csf_og_title);
    	OpenGraph::setUrl(url()->current());
    	SEOMeta::setCanonical(url()->current());
    	OpenGraph::addProperty('keyword', $casestudy->csf_og_keywords);
    	OpenGraph::addProperty('type', 'Project');
    	OpenGraph::addProperty('locale', 'en_GB');
            //OpenGraph::addProperty('locale:alternate', ['pt-pt', 'en-us']);
    	OpenGraph::addImage([$casestudy->csf_og_image, 'size' => 300]);
