   ...
   protected $time;
    ...
    {         
      ...
      $articles = Article::with('authors')->where('home_articles',1)->where('active_flag', 1)->inRandomOrder()->take(6)->get();
        ...
        $strategy=DB::SELECT('CALL Get_strategy()');
        $research_development=DB::SELECT('CALL Get_research_development()');
        $clinical_trials=DB::SELECT('CALL Get_clinical_trials()');
        $information_technology=DB::SELECT('CALL Get_information_technology()');
        $manufacturing=DB::SELECT('CALL Get_manufacturing()');
        $expert_talk=DB::SELECT('CALL Get_expert_talk()');
        ...
        $facilities=DB::SELECT('CALL Get_facilities()');
         ...
         return view('home/index' ,compact('banners','news','pressreleases','articles','interviews','whitepaper','events','reaserchinsites','casestudy','technotrend','sliders','news','advaisoryboard','strategy','research_development','clinical_trials','information_technology','manufacturing','expert_talk','facilities','ebook'));
    }  
