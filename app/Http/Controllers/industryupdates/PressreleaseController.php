...
class PressreleaseController extends HomeController
{
	...
	{	
		 ...
		 $pressreleases = Pressrelease::where('active_flag',1)->orderBy('id','desc')->select('url','title','date','img_title','short_description')->get()->take(10);		
	...
	{
		$pressreleases = Pressrelease::where('url',$url)->where('active_flag',1)->orderBy('id','desc')->select('title','date','description','id','short_description','created_at','updated_at')->first();
	...
	public function more($offset){
	 ...
	 public function filter($month,$year){
		...
		$data_count = Pressrelease::where('active_flag',1)->whereIn(DB::raw("Month(date)"), [$month])->whereIn(DB::raw("Year(date)"), [$year])->orderBy('id','desc')->select('url','title','date','img_title','short_description')->count(); 
		$banners = $this->banners;
